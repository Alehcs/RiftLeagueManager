...
function buildPlayer(args: {
  ...
  return {
    ...
    nationality: nat,
    age,
    image_url: args.raw?.portrait ?? null,
    avatar_seed: args.raw?.avatar_seed ?? null,
    avatar_style: args.raw?.avatar_style ?? null,
...
function buildLeague(raw: RawLeague, db: Database, opts?: { ownerId?: string; isSeed?: boolean; skipSchedule?: boolean }) {
  ...
  for (const rt of raw.teams) {
    ...
    const team: Team = {
      ...
      name: rt.name,
      short_name: rt.short,
      region: rt.region ?? raw.region,
      country: rt.country,
      tier: rt.tier ?? raw.tier,
      ...
      color_secondary: rt.color_secondary ?? null,
      brand_gradient: rt.brand_gradient ?? null,
      created_at: ts,
      updated_at: ts,
    };
